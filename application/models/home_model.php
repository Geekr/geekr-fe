...
class home_model extends CI_Model {
    ...
    function __construct()
    {
     ...
     {  
		...
		function todos($cuantos= 8, $desde = 0)
    {
		$this->db->where('blog_post.estado',1);
		$this->db->where('blog_post.autor = autor_post.id_autor');
	    $query = $this->db->get('blog_post,autor_post',$cuantos, $desde);
		$this->db->order_by('id','desc');
		...
		function autor_post ($cuantos= 8, $desde = 0) {
		$this->db->where('autor_post.estado_autor',1);
		$this->db->where('autor_post.id_autor = blog_post.autor');
		$query = $this->db->get('autor_post,blog_post',$cuantos, $desde);
		$this->db->order_by('id','desc');
		return $query;
